  ...
  *ierr=-1;
  ...
  if ((type=='S'||type=='H')&&part!='B'&&m==n) // S :symetic  B:both lower and upper are specified!B means either upper or lower are specified
  ...
  else //general pattern
    {
      ...
      int* row = (int*)aligned_malloc((size_t)sizeof(int)*(mat->M+1));
      memset(row,0,sizeof(int)*(mat->M+1));
      memcpy(row,mat->PB,sizeof(int)*mat->M);
      row[mat->M] = mat->PE[mat->M-1];
      rmbv_csr(row,mat->IA1,mat->A,mat->M,mat->K,mat->n_A,x,y);
      aligned_free(row);
