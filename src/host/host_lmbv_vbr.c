...
// **********************************************************************
void dlmbv_vbr (DSPMAT* mat,double* x,int n,double *y,int m,int *ierr)
{
  ...
  *ierr = -1;
  ...
  if ((mat->FIDA!=VBR_FORMAT)||(mat->M!=n)||(mat->K!=m)) {
...
//  end_y = -1;
  ...
  if ((type=='S')&&(!(part=='B'))&&(m==n)) {
      if (part=='L') {
          for(i=0;i<mb;i++)//i is block row id
            {
              pntr = mat->PB[i];//pntr is block id
              while(pntr<mat->PE[i])
                {
                  start_x=mat->BP1[i]+ofs;
                  len_x=mat->BP1[i+1]-mat->BP1[i];

                  //IA1 block colume id to block id
                  start_y = mat->BP2[mat->IA1[pntr+ofs]+ofs] + ofs;
                  len_y = mat->BP2[mat->IA1[pntr+ofs]+ofs+1]-mat->BP2[mat->IA1[pntr+ofs]+ofs];

                  start_a = mat->IA2[pntr+ofs] + ofs;

                  if(i==mat->IA1[pntr+ofs]+ofs)
                    dblock_T_mult_vec (&(mat->A[start_a]),&(x[start_x]),len_x,&(y[start_y]),len_y,store,ierr);
                  else  if (i>mat->IA1[pntr+ofs] + ofs)
                    {
                      dblock_T_mult_vec (&(mat->A[start_a]),&(x[start_x]),len_x,&(y[start_y]),len_y,store,ierr);
                      dblock_mult_vec(&(mat->A[start_a]),&(x[start_y]),len_y,&(y[start_x]),len_x,store,ierr);
                    }
                  pntr++;
                }
            }
                  ...
                  while(pntr<mat->PE[i])
                    {
                      ...
                      if(i==mat->IA1[pntr+ofs]+ofs)
                        ...
                        {
                          dblock_T_mult_vec (&(mat->A[start_a]),&(x[start_x]),len_x,&(y[start_y]),len_y,store,ierr);
                          dblock_Z_mult_vec(&(mat->A[start_a]),&(x[start_y]),len_y,&(y[start_x]),len_x,store,ierr);
                        }
                      pntr++;
                    }
                }
